import subprocess
from PIL import Image
from pathlib import Path
from bf1942.shell import eprint

PATHMAP_TYPES = [
    'Tank0Level0Map',
    'Infantry1Level0Map',
    'Boat2Level2Map',
    'LandingCraft3Level0Map',
    'Car4Level0Map'
]

def genpathmaps(src, dst):
    src_path = Path(src)

    mode = '-B' if src_path.suffix == '.raw' else '-8'

    exe_path = Path(__file__).parent.parent / 'tools' / 'genpathmaps'
    ret = subprocess.run([exe_path, '-v', mode, src_path, dst], capture_output=True, text=True)
    return False if ret.returncode > 0 else True

# from bitmap to raw: input.bmp -8
# from raw to bitmap: input.raw -B

def convert_image(source_file, destination_file):
    try:
        with Image.open(source_file) as im:
            im.save(destination_file)
    except OSError:
        return False
    return True

def convert_pathmap(src, dst, in_fmt, out_fmt, ovr):
    source_path = Path(src)
    destination_path = Path(dst)

    pm_types = [f'{f}.{in_fmt}' for f in PATHMAP_TYPES]
    in_pms = [f for f in source_path.iterdir() if f.is_file() and f.name in pm_types]

    for in_pm in in_pms:
        dst_item = destination_path / f'{in_pm.stem}.{out_fmt}'

        if dst_item.exists() and ovr is False:
            print(f'pathmap: skip: {dst_item.name}')
            continue

        if dst_item.exists():
            print(f'pathmap: overwrite: {dst_item.name}')
        else:
            print(f'pathmap: convert: {dst_item.name}')

        if genpathmaps(in_pm, destination_path) is False:
            eprint(f'pathmap: error converting "{in_pm}"')

        # TODO delete extra .bmp files generated by genpathmaps

        # if in_fmt == raw -> run genpathmaps, image_convert if output is png
        # if out_fmt == raw -> image_convert if input is png, run genpathmaps

        # if convert_image(src_item, dst_item) is False:
        #     return False