...
def genpathmaps(src, dst):
    ...
    mode_switch = '-B' if mode == GENPATHMAPS_BMP_MODE else '-8'
    mode_suffix = '.bmp' if mode == GENPATHMAPS_BMP_MODE else '.raw'

    exe_path = Path(__file__).parent.parent / 'tools' / 'genpathmaps'

    with TemporaryDirectory() as tmp:
        ret = subprocess.run([exe_path, '-v', mode_switch, src_path, tmp], capture_output=True, text=True)
        if ret.returncode > 0:
            return False

        out_path = Path(tmp) / (src_path.stem + mode_suffix)

        if dst_path.suffix == '.png':
            # TODO convert to png if dst suffix is png

        shutil.copyfile(out_path, dst_path)

    return True

# from bitmap to raw: input.bmp -8
# from raw to bitmap: input.raw -B
...
def convert_pathmap(src, dst, in_fmt, out_fmt, ovr):
    ...
    for in_pm in in_pms:
        ...
        # TODO delete extra .bmp files generated by genpathmaps

        # if in_fmt == raw -> run genpathmaps, image_convert if output is png
        # if out_fmt == raw -> image_convert if input is png, run genpathmaps
